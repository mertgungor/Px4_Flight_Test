...
int main(int argc, char **argv){
    ...
    ros::Rate water_taking(0.2);//waiting untill drone takes water or drone pours water
    ...
    position water = {30,2*r1,1,false};
    position fire = {0,2*r1,1,false};
    ...
    while(ros::ok()){
        ...
        drone.moveGlobal(pos2);
        waitUntillX(pos2.x, drone);

        if(lap == 2){//2nd lap

            drone.moveGlobal(water);
            ROS_INFO("TAKING WATER...");
            waitUntillX(water.x , drone);
            water_taking.sleep();

            drone.moveGlobal(fire);
            ROS_INFO("POURING WATER");
            waitUntillX(fire.x, drone);
            water_taking.sleep();
